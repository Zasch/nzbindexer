...
class SubjectExtracter {
    ...
    cleanDuplicateFilenames() {
        if (this.record.filename) {
            ...
            splitted.forEach((part) => {
                if (this.record.filename.indexOf(part) === -1)
                    found = false;
            });
            if (found) {
                return this.record.newsubject = '';
    ...
    isOk() {
        ...
        if (this.record.regex.match(/^(?=(?:.{25}|.{30}|.{32})$)[0-9a-z_]*$/)) {
            // return this.record.filter = true;
        ...
        // 	return this.record.filter = true;
        // }
        if (this.record.regex.match(/^[0-9a-z_]{10}$/)) {
            // return this.record.filter = true;
            return this.record.spam = true;
        }
    }
    printErrors() {
        if (!this.record.filter) {
            const print = [];
            print.forEach((error) => {
                if (this.record.errors[error])
                    log.error(error, this.record.errors[error]);
                // if (this.record.errors[error]) console.log(this);
            });
        }
    }
}
exports.SubjectExtracter = SubjectExtracter;
// const record = new SubjectExtracter('100_125_250_600_83 - 03 of 52 - 100_125_250_600_083.part01.rar (040/274)');
// console.log(record);
//# sourceMappingURL=subjectextracter.js.map