...
class NitpinWorker extends EventEmitter {
    constructor(parent) {
        ...
        super(); // eventemitter
        var that = this, sockType = require(parent.secure ? 'tls' : 'net'), attempts = 0, socket;
    ...
    _submit(command, multiline, compressed, callback) {
        var that = this, socket = this.socket, pipe = socket;
    ...
    parseArticle(article, hasHead, hasBody) {
        var headers, lastkey, lines, head, temp, body, id;
    ...
    _stat(group, id, callback) {
        ...
        that._group(group, false, () => {
            var cmd = 'STAT ', art = that.normalizeArticleId(id);
            if (art.error) {
    ...
    _getHead(group, id, callback) {
        ...
        Fn.series(function changeGroup(next) {
            ...
            var cmd = 'HEAD ', art = that.normalizeArticleId(id);
            if (art.error) {
                return callback(new Error(art.error));
            }
            cmd += art.id;
    ...
    _getBody(group, id, callback) {
        ...
        Fn.series(function changeGroup(next) {
            ...
            var cmd = 'BODY ', art = that.normalizeArticleId(id);
            if (art.error) {
                return callback(new Error(art.error));
            }
            cmd += art.id;
    ...
    _getArticle(group, id, callback) {
        ...
        Fn.series(function changeGroup(next) {
            ...
            var cmd = 'ARTICLE ', art = that.normalizeArticleId(id);
            if (art.error) {
                return callback(new Error(art.error));
            }
            cmd += art.id;
    ...
    _format(callback) {
        var that = this, val = this.info('listformat');
        if (val) {
    ...
    _over(group, first, last, callback) {
        var that = this, hasXzver = this.info('cap.xzver'), format, lines, r;
        async.series([
            ...
            var results, record, field, temp, line, conf, i, j;
            if (err) {
                return callback(err);
            }
            results = [];
...
//# sourceMappingURL=nitpin_worker.js.map