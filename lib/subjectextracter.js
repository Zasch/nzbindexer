...
class SubjectExtracter {
	...
	cleanWhiteSpace() {
		this.record.newsubject = this.record.newsubject.replace(/  /g, " ");
		this.record.newsubject = this.record.newsubject.trim();
	}
	...
	extractRegex() {
		...
		if (filename && extension) {
			...
			if (filename_without_extension) {
				...
				if (fn) {
					this.record['regex'] = filename_without_extension.replace(regex, '').replace(/(-sample|\.sample|\.rar)$/, '').trim();
				} else {
					this.record['regex'] = filename_without_extension.replace(/(-sample|\.sample|\.rar)$/, '').trim();
				}
				...
				this.record['regex'] = this.record['regex'].replace(/([-_.,]thumbs|\.?\s?screen|\.r|\.f)$/, '')
				if (this.record['regex'] === '') {
					return this.record.errors['extractRegex'] = filename_without_extension;
				}
				return;
			} else {
	...
	cleanStrings() {
		// this.record.newsubject = this.record.newsubject.replace(/\"(.*)\"/, '$1')
		this.record.newsubject = this.record.newsubject.replace('join us @', '');
		this.record.newsubject = this.record.newsubject.replace('<<best provider hitnews>>', '');
		this.record.newsubject = this.record.newsubject.replace('<3 https://house-of-porn.com number one porn source <3', '');
		this.record.newsubject = this.record.newsubject.replace('https://house-of-porn.com/', '');
		this.record.newsubject = this.record.newsubject.replace('brothers-of-usenet.net', '');
		this.record.newsubject = this.record.newsubject.replace('empfehlen useindex.net/', '');
		this.record.newsubject = this.record.newsubject.replace('free invites by b-deadangel', '');
		this.record.newsubject = this.record.newsubject.replace('>>>no.1 nzb indexer<<<', '');
		this.record.newsubject = this.record.newsubject.replace('<<house-of-usenet.com>>', '');
		this.record.newsubject = this.record.newsubject.replace('<<house-of-usenet>>', '');
		this.record.newsubject = this.record.newsubject.replace('<<<hitnews.com>>>', '');
		this.record.newsubject = this.record.newsubject.replace('<<hitnews>>', '');
		this.record.newsubject = this.record.newsubject.replace('www.usenet-space-cowboys.online', '');
		this.record.newsubject = this.record.newsubject.replace('usenet-space-cowboys.online', '');
		this.record.newsubject = this.record.newsubject.replace('[u4a]', '');
		this.record.newsubject = this.record.newsubject.replace('- description -', '');
		this.record.newsubject = this.record.newsubject.replace('-always take the post from the first and original poster!-', '');
		// if (this.record.newsubject.indexOf('empfehlen') !== -1) {
		// 	console.log(this.record.newsubject);
		// }
		this.record.newsubject = this.record.newsubject.replace('<-> empfehlen <->', '');
		this.record.newsubject = this.record.newsubject.replace('-[#a.b.erotica@efnet]', '');
		this.record.newsubject = this.record.newsubject.replace('-[full]', '');
		this.record.newsubject = this.record.newsubject.replace(/ - /g, ' ');
		this.record.newsubject = this.record.newsubject.replace(/ -/g, ' ');
		this.record.newsubject = this.record.newsubject.replace(/- /g, ' ');
		this.record.newsubject = this.record.newsubject.replace(/-\[/g, ' [');
		...
		this.record.newsubject = this.record.newsubject.replace(/ \]/g, ']');
		this.record.newsubject = this.record.newsubject.replace(/\[\]/g, ' ');
		this.record.newsubject = this.record.newsubject.replace(/ yenc/g, '');
		this.record.newsubject = this.record.newsubject.replace(/yenc /g, '');
		this.record.newsubject = this.record.newsubject.replace(/^\[\d{6}\]/, '');
		this.cleanWhiteSpace();
	}
	...
	isOk() {
		...
		if (this.record.regex.match(/^[0-9a-z]{10}$/)) { // this *could* filter stuff that we DO want right?
			this.record.filter = true;
		}
	}
	printErrors() {
