String.prototype.startsWith = function(prefix) {
    return this.indexOf(prefix) == 0;
};

String.prototype.endsWith = function(suffix) {
    return this.indexOf(suffix, this.length - suffix.length) !== -1;
};

var fib = [];
for (var i = 0, inc = 0; i < 2000; i++) {
    inc += i;
    fib.push(inc);
}

(function(exports) {

    exports.addMinutes = function(date, minutes) {
        return new Date(date.getTime() + (minutes * 60000));
    };

    // Extract filename from HEADER
    exports.extractFilename = function(subject) {
        var match = subject.match(/^.*\"(.*)\".*/);
        // console.log("match1", match);
        var filename = (match && match[1] ? match[1] : undefined);
        // console.log("filename", filename);

        if (!filename) {
            match = subject.match(/^([a-z0-9\.-]*).rar/i); // a-zA-Z0-9
            // console.log("match2", match);
            filename = (match && match[1] ? match[1] + ".rar" : undefined)
                // console.log("filename", filename);
        }
        if (filename == undefined) {
            match = subject.match(/^([a-z0-9\.\+-]*).par2/i); // a-zA-Z0-9
            // console.log("match3", match);
            filename = (match && match[1] ? match[1] + ".par2" : undefined)
                // console.log("filename", filename);
        }
        if (filename == undefined) {
            global.log.error("filename missing:", subject);
        }
        return filename;
    };

    // Extract Release ID from HEADER
    exports.extractReleaseID = function(subject) {
        var match = subject.match(/^\[(\d+)\]/);
        var releaseid = null;
        if (match && match[1]) {
            var releaseid = match[1];
            // console.log(releaseid);

        }
        // console.log("releaseid", releaseid);
        return releaseid;
    };

    // Extract Part & Total (per file bases) from HEADER
    exports.extractpartAndTotal = function(subject) {
        var part = null;
        var total = null;

        var match = subject.match(/^.+yEnc.+\((\d+)\/(\d+)\)/);
        if (match && match[1] && match[2]) {
            part = parseInt(match[1]);
            total = parseInt(match[2]);
        } else {
            match = subject.match(/^.+\((\d+)\/(\d+)\)/);
            if (match && match[1] && match[2]) {
                part = parseInt(match[1]);
                total = parseInt(match[2]);
            }
        }
        if (part == null) {
            console.error("part missing:", part, total, subject);
        }
        return {
            part: part,
            total: total
        };
    };

    // Remove Part & Total (per file bases) from subject when inserting in files table
    exports.extractpartAndTotalString = function(subject) {
        // console.log("before", subject);
        var part = null;

        var match = subject.match(/^.+(yEnc.+\(\d+\/\d+\))/);
        if (match && match[1]) {
            part = match[1];
        } else {
            match = subject.match(/^.+(\(\d+\/\d+\))/);
            if (match && match[1]) {
                part = match[1];
            }
        }
        if (part == null) {
            global.log.error("part missing:", part, subject);
        }
        // console.log(part, subject.replace(part, "").trim());
        return subject.replace(part, "").trim();
    };

    exports.htmlEscape = function(str) {
        return String(str)
            .replace(/&/g, '&amp;')
            .replace(/"/g, '&quot;')
            .replace(/'/g, '&#39;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;');
    };

    exports.getFileNZB = function(rows) {
        var nzb = '';
        nzb += '<file poster="' + rows[0].poster + '" date="' + Math.round(rows[0].postdate.getTime() / 1000) + '" subject="' + this.htmlEscape(rows[0].subject) + '">\n';
        nzb += '<groups>\n';
        nzb += '<group>alt.binaries.erotica</group>\n';
        nzb += '</groups>\n';
        nzb += '<segments>\n';

        rows.forEach(function(row) {
            var line = '<segment bytes="' + row.bytes + '" number="' + row.part + '">' + row.articleid + '</segment>';
            nzb += line + '\n'
        });

        nzb += '</segments>\n';
        nzb += '</file>\n';
        return nzb;
    };

    exports.getFileNZBnew = function(rows) {
        var nzb = {
            group: 'alt.binaries.erotica',
            poster: rows[0].poster,
            postdate: Math.round(rows[0].postdate.getTime() / 1000),
            subject: this.htmlEscape(rows[0].subject),
            segments: []
        };
        rows.forEach(function(row) {
            nzb.segments.push({
                bytes: row.bytes,
                part: row.part,
                articleid: row.articleid
            });
        });
        // console.log(nzb);
        // console.log();
        return nzb;
    };

    exports.getReleaseNZB = function(rows) {
        var nzb = '';
        nzb += '<?xml version="1.0" encoding="iso-8859-1" ?>\n';
        nzb += '<!DOCTYPE nzb PUBLIC "-//newzBin//DTD NZB 1.0//EN" "http://www.nzbindex.com/nzb-1.0.dtd">\n';
        nzb += '<nzb xmlns="http://www.newzbin.com/DTD/2003/nzb">\n';

        rows.forEach(function(row) {
            nzb += row.nzb;
        });

        nzb += '</nzb>\n';
        // console.log(nzb);
        // console.log();
        return nzb;
    }

    exports.getReleaseNZBnew = function(rows) {
        var nzb = [];
        rows.forEach(function(row) {
            nzb.push(row.nzb);
        });
        // console.log(nzb);
        // console.log();
        return nzb;
    }

    exports.fileComplete = function(rows) {
        var articles = [];
        var partsum = 0;
        if (rows[0] && rows[0].total) {
            var total = rows[0].total;
            rows.forEach(function(row, index) {
                // console.log(row.part);
                // if (index == 0) total = row.total;
                partsum += row.part;
            });
            return partsum == fib[total]; // correct for (1) extra part
            // return (partsum == fib[total]) || (partsum < fib[total] + total); // correct for (1) extra part
        } else {
            console.log("no total??", rows);
            return false;
        }
    };

    exports.howMuchComplete = function(rows) {
        var total = rows[0].total;
        var percentage = Math.floor(100 * rows.length / total);
        if (percentage < 100) {
            // console.log("file", rows[0].filename, "is NOT complete");
            // while(!newestdate && !oldestdate) {
            // console.log("test");
            // }
            var numhoursfromlatest = Math.floor((newestdate - rows[0].postdate) / (1000 * 3600));
            var numhoursfromfirst = Math.floor((rows[0].postdate - oldestdate) / (1000 * 3600));
            if (numhoursfromfirst > 48 && numhoursfromlatest > 48) {
                //move files
            }
            // console.log("age", numhoursfromfirst, numhoursfromlatest);
            // console.log("rows", rows.length, "total", total, "percentage", percentage, total - rows.length, "missing\n");
        }
    };

    exports.extractReleasePartAndTotal = function(row) {
        var match = row.subject.match(/^.*\[(\d+)\/(\d+)\]/);
        var retval = {
            part: null,
            total: null
        };
        // File 1 of 9
        if (match) {
            retval = {
                part: parseInt(match[1]),
                total: parseInt(match[2])
            };
        }
        if (!retval.total) {
            match = row.subject.match(/^.+File\s(\d+)\sof\s(\d+)/);
            if (match) {
                retval = {
                    part: parseInt(match[1]),
                    total: parseInt(match[2])
                };
            }
        }
        if (!retval.total) {
            match = row.subject.match(/^\((\d+)\/(\d+)\)/);
            if (match) {
                retval = {
                    part: parseInt(match[1]),
                    total: parseInt(match[2])
                };
            }
        }
        if (!retval.total) {
            match = row.subject.match(/^.*\[(\d+) of (\d+)\]/);
            if (match) {
                retval = {
                    part: parseInt(match[1]),
                    total: parseInt(match[2])
                };
            }
        }
        return retval;
    };

    exports.getMatcherString = function(filename) {
        var m = filename.split(".");
        // console.log(filename, m);
        m.pop();
        if (m[m.length - 1]) {
            var isVol = m[m.length - 1].match(/^vol[0-9]+\+[0-9]+/);
            if (isVol) {
                m.pop();
            }
            var isPart = m[m.length - 1].match(/^part[0-9]+/);
            if (isPart) {
                m.pop();
            }
        }
        return m.join(".");
    };

    exports.calcTotalFileBytes = function(rows) {
        var bytes = 0;
        rows.forEach(function(row) {
            bytes += row.bytes;
        });
        return bytes;
    };

    exports.fileIDS = function(rows) {
        var files = [];
        rows.forEach(function(row) {
            files.push('"' + row.id + '"');
        });
        return files;
    };

    exports.mongoIDs = function(rows) {
        // console.log("mongoIDs rowcount", rows.length);
        var files = [];
        rows.forEach(function(row) {
            // console.log(row);
            files.push(row._id);
        });
        // console.log(files);
        return files;
    };

    exports.mongoIDsWaterline = function(rows) {
        // console.log("mongoIDsWaterline rowcount", rows.length);
        var files = [];
        rows.forEach(function(row) {
            // console.log(row);
            files.push(row.id);
        });
        // console.log(files);
        return files;
    };

    exports.getFiletypes = function(rows) {
        var files = {};
        rows.forEach(function(row) {
            if (row.filetype) {
                if (!files[row.filetype]) {
                    files[row.filetype] = 0
                };
                files[row.filetype] += 1;
            } else {
                global.log.error("filetype missing", row.filename);
            };
        });
        return files;
    };

})(exports);
